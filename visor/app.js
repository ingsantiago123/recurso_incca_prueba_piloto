 ...
 *                               y arma el mismo objeto "datos" que antes.
 *   2) renderizarInterfaz(datos) → pinta el HTML con esos datos.
 *
 * Si la URL no trae "curso" (por ejemplo, abriste la página a mano sin
 * parámetros), se muestra un recurso de EJEMPLO con un banner que lo
 * avisa, solo para que siempre haya algo que ver mientras pruebas.
 * ------------------------------------------------------------------- */
(function () {
  "use strict";
  ...
  const EJEMPLO_DEMO = {
    ...
    profesor_img_url: "https://ui-avatars.com/api/?name=Pepito+Perez&background=0B349D&color=fff&size=200&bold=true",
  ...
  function renderizarInterfaz(datos) {
    ...
    avatar.src = datos.profesor_img_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(datos.profesor_nombre || "Docente")}&background=E2E6E9&color=0B349D&size=200`;
    ...
    const videoCard = $("#videoFrame").closest(".card--video");
    if (datos.video_url) {
